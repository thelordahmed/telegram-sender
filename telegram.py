...
class Telegram:
    ...
    username_notFound_modal = '//div[@class="error_modal_wrap md_simple_modal_wrap"]'
    ...
    def search_phone(cls, number: str) -> None or False:
        ...
        # clicking x button
        try:
            cls._window.find_element_by_xpath(cls.x_button).click()
        except Exception as e:
            print(e)
            pass
        cls._window.find_element_by_xpath(cls.search_input).send_keys(number)
        sleep(1)
        # waiting for first result to appear - maximum 10 seconds
        try:
            WebDriverWait(cls._window, 10).until(ec.visibility_of_element_located((By.XPATH, cls.first_result)))
        except TimeoutException:
            return False
        cls._window.find_element_by_xpath(cls.search_input).send_keys(Keys.ENTER)
    ...
    @classmethod
    def sending_attachment(cls, paths):
